...
if len(df) > 0:
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        print(f"#{i+1:<2d}  {row['filename_prefix']:20s}  {row['genome_version']}  "
              f"{int(row['c840_reads_with_smn1_base_C']):5,d} out of {int(row['c840_total_reads']):5,d} reads")
        for k, v in row.to_dict().items():
            if k not in df_columns and not pd.isnull(v):
                print(f"    {k}: {v}")
else:
    print("No new cases")
